...
```
user の発言
   ├─→ jev:  この発言を受けて自分の感情がどう動くべきかを8軸で返す
   └─→ LLM:  同じ問いに同じ入力で答える
...
**両者が受け取るのは常に user の発言だけ**とする。状態が分岐するのは判定が違うからであって、
入力が違うからではない、と言い切れる形にするため。

## 判定タスク

両者に同一の課題を与える。

> user の発言を読み、それを受けて自分の感情が8軸それぞれどう動くかを符号付きで答える
...
判定対象は user の発言のみ。記録された返答は入力に含めない。画面に出ているものがそのまま入力になる。
