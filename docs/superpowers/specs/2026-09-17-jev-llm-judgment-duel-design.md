...
`confidence` は応答に含まれるが**画面には出さない**（下記「判定の対比（中央）」参照）。jev が返す値ではなくこちらの算出値であるため。
...
人格は画面で選ぶ。送信のたびに、そのとき選ばれている人格が両側の返答生成に渡る。

### 入力は user の発言だけ

**会話ログの再生機能は持たない。** user の発言は毎回ユーザーがテキスト欄に書く。
記録された返答や当時の自己申告を入力に使う設計は、データの形に引きずられたものであり、
上の流れとは別物だった。判定に渡るのは、現在の8軸と、いまテキスト欄にある発言のみ。

中央パネルに置くのは user の発言の入力欄と判定のずれのみ。
...
レイアウトは3カラム。中央に user の発言、左に LLM、右に jev。

```
┌─────────────────────────────────────────────────────┐
│ 人格 [friendly]                              reset  │
├──────────────┬──────────────────┬───────────────────┤
│ LLM + モデル選択│ user 発言（入力）│ jev              │
│ プルチックの輪│                  │ プルチックの輪     │
...
**左右のパネルにゲージは置かない。** 判定の差は中央に8軸ぶん並べ、左右に分けて対比させる。
合計値ひとつでは「どの軸で割れたのか」が分からないため。

軸ごとに1行。**中央が 0、左右の末端が 1.0。** 左半分が LLM の変動値、右半分が jev の変動値で、
棒の長さがその軸をどれだけ動かしたかを表す。中央にその軸の差の数値を出す。
...
**符号は色で表す。** 棒の長さは変動の大きさ（絶対値）で、向きは色が持つ。
正はその側の色（LLM は `#ffb84d`、jev は `#4dffb8`）、負はくすんだ灰。数値にも符号を付ける。
長さだけでは、LLM が −0.5 で jev が +0.5 のときに同じに見えてしまうため。

**確信度は画面に出さない。** かつては jev の `probabilities` から正規化エントロピーで算出した値を
「確信度」として点線で重ねていたが、削除した。**jev はその値を返していない。** AI SDK 経由の回答型に
`confidence` フィールドは存在せず（型定義と実応答の双方で確認済み）、返るのは `score` と
`probabilities` だけである。こちらが計算した値を、モデルが言ったかのように見せることになるため出さない。

軸名は各行の**中央**に置く。差の数値も中央。左右の棒が中央を挟んで対称に伸びる構造は保つ。

中央下には従来どおり8軸の差の合計も出す（`判定のずれ（8軸の差の合計）`）。

**差は符号付きの値どうしで計算する。** `|llm − jev|` であって、棒の長さ（絶対値）どうしの差ではない。
LLM が −0.5、jev が +0.5 のとき、差は 1.00 でなければならない。棒の長さから計算すると 0.00 になり、
この画面が最も見せたい「正反対の判断」を見逃す。
...
**折りたたまず常に表示する。** パネルを開いた時点で中身が読める状態にする。
ただし輪・ゲージ・レイテンシ・生成された返答を押しのけないこと。JSON はそれらの下に置き、
枠の高さを固定して自分の枠の中で縦スクロールさせる。入力と出力はどちらがどちらか分かるようにする。

### 操作

**実行の起点は送信ボタンだけ**とする。画面には API を叩く操作が他に無い。

- 中央のテキスト欄に user の発言を書く
- 「メッセージを送信」で判定 → affectus 適用 → 返答生成を実行
- **自動で送信する経路は持たない。** 押した回数と実行回数・課金回数を一致させる
- 人格の選択、LLM 側モデルの選択
- 両状態のリセット
- **判定が飛行中の間は、入力・人格の変更・リセットを受け付けない。** 受け付けると、
...
| 同梱する | 同梱しない |
|---|---|
| `src/` `public/` `scripts/` | `.env.local`（API キー） |
...
- **Gateway 経由の応答に `confidence` は含まれない（確定）**。インストール済みの `ai` と `@ai-sdk/provider` の評価回答型を検査した結果、`confidence` フィールドはどこにも存在しない（`ai` の型定義で該当 0 件。`@ai-sdk/provider` の唯一の出現は boolean 型のコメント「Not confidence in either outcome.」のみ）。回答が持つのは `score` と、**オプショナルな** `probabilities` だけ。したがって確信度は常にこちらの算出値であり、TypeSafe が自社 API で返す `confidence` とは別物である。**そのため画面には出さない。**
